...
//		tv.netflix(); //부모객체의 값은 자식 클래스에 있는 멤버 사용 불가능
		tv.powerOnOff();

		SmartTv smartTv = new SmartTv();
		System.out.println(smartTv);
		smartTv.netflix(); // 자식 객체의 값은 자식 클래스에 있는 멤버 사용가능
		smartTv.chUp(); // 자식 객체의 값으 부모 클래스에 있는 멤버도 사용가능

		// Up casting
		// 방법1
		// SmartTv()는 Tv클래스의 참조 변수 tv1에 할당된다
...
//		tv1.netflix(); //업캐스팅 되면 자식 클래스에 만들어진 멤버는 사용 불가능하다
		...
		// 방법2
		Tv tv2 = (Tv) smartTv;
		// tv2는 smartTv 클래스이 객체를 참조하지만 업캐스팅으로 인해 Tv 타입으로 취급된다
		// 따라서 Tv 클래스의 필드와 메소그에만 접근할 수 있다
