...
//		animal2.walk();
		// 자식클래스에 추가한 멤버는 부모타입의 참조변수로는 접근할 수없다
		// 이 경우 다운 캐스팅을 해야한다
		((Dog) animal2).walk();

		// 객체 생성
		Dog dog = new Dog("별이", 4);
		Cat cat = new Cat("달이", 5);
	...
	// 하나의 메소드에서 매개변수를 부모타입으로 만들면 여러자식 타입을 받아서 사용이 가능한다(다형성)
	static void printTest(Animal ani) {
		System.out.println(ani.name + "이/가 운다");
		ani.crying(); // 업캐스팅되 객체가 들어와도 오버라이딩 된 메소드가 호출된다
	}
//	Animal ani = new Dog();
	...
	static void tryWalk(Animal ani) {
		// 매개변수로 받아온 객체의 주소값이 Cat타입인자, Dog 타입인지 확인한다
		if (ani instanceof Dog) {
			// Dog 타입이 맞다면 Dog로 Down Casting한다
			((Dog) ani).walk();
		} else {
			System.out.println("산책안감");
		}
	}
